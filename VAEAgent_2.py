...
class Agent():
    def __init__(self,vae,tokenizer,actions, verbose=True):
    ...
    def _update_state(self, action_nr, encoded_page):
        ...
        action_response = np.concatenate((np.array([action_nr]),encoded_page))
    ...
    def _analyze_response(self, action, response, reward, learning = True):
        ...
        encoded_page = self.vae.encoder(tokens)[0].numpy()[0]
