...
class mockSQLenv(object):
    ...
    def __init__(self, actions, htmlpages, flag_reward = 10, query_reward = -1, verbose=True,):
    ...
    def step(self,action_number=None,action_string=None):
        ...
        if (action_number==self.setup[0]):
            ...
            return self.htmlpages[1],self.flag_reward,self.termination,'Server response is 3'
            ...
            if(action_number == self.flag_cols*2 + self.setup[1] + 1 or action_number == self.flag_cols*2 + self.setup[1] + 2):
                ...
                return self.htmlpages[0],self.query_reward, self.termination, "Server response is 4"

            if(self.verbose): print('Query has the correct escape, but contains the wrong number of rows. I return 0')
            return self.htmlpages[3],self.query_reward, self.termination,'Server response is 0'
        else:
            if(self.verbose): print('Query is syntactically wrong. I return -1')
            return self.htmlpages[3],self.query_reward, self.termination,'Server response is -1'


    def reset(self):
        self.termination = False
        if(self.verbose): print('Game reset (but not reinitialized with a new random query!)')
        return self.htmlpages[2],0,self.termination,'Game reset'
